...
class System:
    ...
    def __to_json__(self) -> dict:
        return {"type":"system","os_info": self.os_info.__to_json__(), "hostname": self.hostname, "custom_name": self.custom_name, "ip_address": self.ip_address, "mac_address": self.mac_address, "enabled": self.enabled}
...
def construct_from_json(backend_json: dict) -> System:
    return System(oi_construct_from_json(backend_json["os_info"]), hostname= backend_json["hostname"], ip_address= backend_json["ip_address"], custom_name = backend_json["custom_name"], mac_address= backend_json["mac_address"], enabled= backend_json["enabled"])